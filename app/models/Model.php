...
//                $sql = "INSERT INTO $table (id $keys) VALUES (NULL $values)";
	...
	*/
       public function exec($req, $fetch = true) {
        ...
        if ($sql->execute()) {
            if ($this->is_insert_into($sql->queryString)) {
                ...
                $data = array();
                if ($fetch) {
                    $data = array('status' => true, 'response' => $sql->fetchAll(\PDO::FETCH_OBJ));
                } else {
                    $data = array('status' => true, 'message' => 'Execute successfully.');
                }
                return $data;
    ...
    /**
	* Check if it is a INSERT INTO query
